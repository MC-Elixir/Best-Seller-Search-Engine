...
用法:
    python main.py                              # 默认 amazon + temu，mock 各 10 条
    python main.py --platforms amazon
    python main.py --limit 100 --offers 10      # 生产模式 需 USE_MOCK_DATA=false
    python main.py --schedule                    # 启动常驻调度器
    python main.py --once                        # 单次执行 (生产参数)
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--platforms", nargs="+", default=["amazon", "temu"],
        choices=["amazon", "temu"],
        help="要抓取的平台"
    )
    ...
    # 调度模式
    parser.add_argument("--schedule", action="store_true", help="启动常驻调度器")
    parser.add_argument("--once", action="store_true", help="单次执行 (生产参数: limit=100, offers=10)")
...
def main() -> None:
    ...
    # --once: 单次生产执行
    if args.once:
        args.limit = args.limit if args.limit != 10 else 100
        args.offers = args.offers if args.offers != 5 else 10
